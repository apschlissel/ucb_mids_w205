...
- Question 1:  Which 5  zip codes have the most trips?
  * Answer: 94105, 94133, 94103
  
  * SQL query:
```sql
	SELECT  zip_code, COUNT(trip_id)
	FROM
...
- Question 2: What hours do we have the most trips?
  * Answer: 8,17
  * SQL query:
```sql
  SELECT COUNT(trip_id) as tcount,
  (EXTRACT (HOUR FROM start_date) ) AS thour
  FROM
	`bigquery-public-data.san_francisco.bikeshare_trips`
	GROUP BY thour
...
- Question 3: what days have most accumulated trips?
  * Answer: 3,4 (Tuesday, Wednesday)
  * SQL query:
```sql
  SELECT  count(trip_id) AS ts,
  (EXTRACT (DAYOFWEEK FROM start_date) ) AS tday
  FROM
	`bigquery-public-data.san_francisco.bikeshare_trips`
	GROUP BY tday
	ORDER BY ts
...
- Question 1:  When do we have bikes available. Average per hour, per station


- Question 2:  How frequently does different subscriber types ride between pairs of stations?

- Question 3: Are there times  of day (on average) when some stations have no or many bikes available?

- Question 4: What is the average trip length between stations?

- Question 5: What would happen if we reduce the included time for Subscribers to 15 min?
...
```sql
  ...
  (EXTRACT (HOUR FROM time) ) AS thour
  FROM
    `bigquery-public-data.san_francisco.bikeshare_status`
  GROUP BY station_id, thour
```
Use pivot table heatmap to explore data.
...
```sql
  SELECT  start_station_id, end_station_id, subscriber_type, COUNT(*),
  FROM
    `bigquery-public-data.san_francisco.bikeshare_trips`
  GROUP BY subscriber_type, start_station_id, end_station_id
```
Use pivot table heatmap to explore data.
...
```sql
  SELECT  station_id, AVG(bikes_available) AS ba, AVG(docks_available) as da,
  (EXTRACT (HOUR FROM time) ) AS thour
  FROM
    `bigquery-public-data.san_francisco.bikeshare_status`
  GROUP BY station_id, thour
...
Use pivot table heatmap to explore, row: thour, colum: station_id, metric ba/da
...
- Question 5:
  * Answer: We could charge for an additional 60K 15 min periods. Question is what it does to number of subcribers, will they leave?
  * SQL query:
...
```sql
  ...
  FROM
    `bigquery-public-data.san_francisco.bikeshare_trips`
  WHERE ROUND(duration_sec/60) < 45 AND subscriber_type = "Subscriber"
